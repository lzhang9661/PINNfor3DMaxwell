...
def plot_waveguide(test_data,test_predict,config):
    k = config['eigenmode']
    axis_size = config['axis_size']
    ...
    fig,ax = plt.subplots(1,3,sharex=True,sharey=True)
    ax = ax.flatten()
    waveguide_port = np.sin(k * np.pi * y / 2.) * np.sin(k * np.pi * z / 2.)

    Ex = test_predict[0,:,:, 0]
    Ey = test_predict[0,:,:, 1]
    Ez = test_predict[0,:,:, 2]

    diff = Ez - waveguide_port  

    norm1 = matplotlib.colors.Normalize(vmin=0,vmax=1)

    ax[0].contourf(y,z,waveguide_port,100,cmap="jet") #
    im1 = ax[1].contourf(y,z,Ez,100,cmap="jet")
    im2 = ax[2].contourf(y,z,diff,100,cmap="binary")
    ...
    fig.colorbar(im1,ax=[ax[0], ax[1]],orientation='horizontal',ticks=FixedLocator([0,0.5,0.99]),fraction=0.02,pad=0.1,cax=cbar_ax1) #原型是plt.Figure
    fig.colorbar(im2,ax=[ax[2]],orientation='horizontal',ticks=FixedLocator([-0.01,0,0.01]),fraction=0.02,pad=0.1,cax=cbar_ax2) #原型是plt.Figure
    ...
    im = ax.contourf(y,z,Ex,100,norm=norm1,cmap="jet")
    ax.set_aspect(1)
    ...
    fig,ax = plt.subplots(1,1,sharex=True,sharey=True)
    im = ax.contourf(y,z,Ey,100,norm=norm1,cmap="jet")
    ax.set_aspect(1)
    fig.subplots_adjust(left=0.3)
    fig.subplots_adjust(right=0.7)
